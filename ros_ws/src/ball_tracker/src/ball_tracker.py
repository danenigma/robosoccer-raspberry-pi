...
class BallTracker:
	...
	buffer_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)  # Convert to HSV
	...
	buffer_frame = cv2.dilate(buffer_frame, np.ones((20, 20), "uint8"))  # Dilate
        cv2.imshow('detect',buffer_frame)
	"""
    ...
    def callback(self, data):
	    ...
	    print(time.time()-tp)
            ...
            print e
    def hsv(self ,frame):
	buffer_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
	cv2.imshow('hsv',buffer_frame)
